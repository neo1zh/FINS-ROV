...
void IMU::Init() {
    ...
    Kalman_Regulator_t _axkalman{
            .filterValue = 0,// 测量的初始值
            .A = 1,  //标量卡尔曼
            .H = 1, //
            .Q = 0.0009,    //预测（过程）噪声方差 影响收敛速率，可以根据实际需求给出
			      .R = 0.0009,    //测量（观测）噪声方差 可以通过实验手段获得
			      .P = 10  //后验状态估计值误差的方差的初始值（不要为0问题不大）
            
    };
    ax_kalman.KalmanInfo = _axkalman;

    Kalman_Regulator_t _aykalman{
            .filterValue = 0,// 测量的初始值
            .A = 1,  //标量卡尔曼
            .H = 1, //
            .Q = 0.0009,    //预测（过程）噪声方差 影响收敛速率，可以根据实际需求给出
    ...
    ay_kalman.KalmanInfo = _aykalman;

    Kalman_Regulator_t _azkalman{
            .filterValue = 0,// 测量的初始值
            .A = 1,  //标量卡尔曼
            .H = 1, //
            .Q = 0.0009,    //预测（过程）噪声方差 影响收敛速率，可以根据实际需求给出
    ...
    az_kalman.KalmanInfo = _azkalman;
...
void IMU::Handle() {
    ...
    /*if(axdata_index<1000){
      axdata[axdata_index]=proData.accel[0];
      axdata_index++;
    }
    else if(axdata_index==1000){
			  axdata_index++;
        float average=0;
        variance=0;
        for(int i=0;i<1000;++i) {
             average += axdata[i]/1000;
        }
        for(int j=0;j<1000;j++){
             variance += (axdata[j]-average)*(axdata[j]-average)/1000;
        }
    }*/
    /*proData.accel[0]=ax_kalman.KalmanFilter(&ax_kalman.KalmanInfo, proData.accel[0]);
    proData.accel[1]=ay_kalman.KalmanFilter(&ay_kalman.KalmanInfo, proData.accel[1]);
    proData.accel[2]=az_kalman.KalmanFilter(&az_kalman.KalmanInfo, proData.accel[2]);*/
